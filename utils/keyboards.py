...
def get_edit_field_keyboard(expense_id: int, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(text=t(lang, "keyboard.edit_category"), callback_data=f"expense_edit_category:{expense_id}")],
        [InlineKeyboardButton(text=t(lang, "keyboard.edit_description"), callback_data=f"expense_edit_description:{expense_id}")],
...
def get_category_keyboard(expense_id: int, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    for i, category in enumerate(EXPENSE_CATEGORIES):
        if i % 2 == 0:
            row = [InlineKeyboardButton(text=t_category(lang, category), callback_data=f"expense_category_select:{category}")]
            if i + 1 < len(EXPENSE_CATEGORIES):
                row.append(InlineKeyboardButton(text=t_category(lang, EXPENSE_CATEGORIES[i + 1]), callback_data=f"expense_category_select:{EXPENSE_CATEGORIES[i + 1]}"))
            keyboard.append(row)
    keyboard.append([
...
def get_pending_expense_category_keyboard(lang: str = "en") -> InlineKeyboardMarkup:
    ...
    for i, category in enumerate(EXPENSE_CATEGORIES):
        if i % 2 == 0:
            row = [InlineKeyboardButton(text=t_category(lang, category), callback_data=f"pending_expense_category:{category}")]
            if i + 1 < len(EXPENSE_CATEGORIES):
                next_category = EXPENSE_CATEGORIES[i + 1]
                row.append(InlineKeyboardButton(text=t_category(lang, next_category), callback_data=f"pending_expense_category:{next_category}"))
            keyboard.append(row)
...
def get_delete_confirmation_keyboard(expense_id: int, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(text=t(lang, "keyboard.confirm_delete"), callback_data=f"expense_confirm_delete:{expense_id}"),
...
def get_description_edit_keyboard(expense_id: int, lang: str = "en") -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(text=t(lang, "keyboard.clear_description"), callback_data=f"expense_clear_description:{expense_id}"),
            InlineKeyboardButton(text=t(lang, "common.cancel"), callback_data=f"expense_select:{expense_id}")
        ]
    ]
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
